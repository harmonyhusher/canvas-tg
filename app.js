...
function initYAxis(context, yMin, yMax) {
  ...
  context.beginPath();
  context.strokeStyle = STROKE_STYLE_AXYS;
  context.font = "20px Arial";
  context.fillStyle = "#000";

  for (let i = 0; i <= ROWS_COUNT; i++) {
    const y = stepY * i;
    const adjustedY = DPI_HEIGHT - PADDING - y;

    const text = yMin + stepText * i;
    context.fillText(text.toFixed(0), 5, adjustedY - 5);
    context.moveTo(0, adjustedY);
    context.lineTo(DPI_WIDTH, adjustedY);
  }
  context.stroke();
  context.closePath();
}
...
function chart(canvas, data) {
  ...
  const [yMin, yMax] = computeBoundaries(chartData);
